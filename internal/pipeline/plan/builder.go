...
import (
	...
	"github.com/pipectl/pipectl/internal/engine/steps/normalize"
	"github.com/pipectl/pipectl/internal/engine/steps/redact"
	"github.com/pipectl/pipectl/internal/engine/steps/rename"
	"github.com/pipectl/pipectl/internal/engine/steps/select"
	_sort "github.com/pipectl/pipectl/internal/engine/steps/sort"
	"github.com/pipectl/pipectl/internal/engine/steps/validatejson"
...
func buildFilterConditions(conditions []spec.FilterCondition, onMissing string, caseSensitive bool) ([]*filter.Condition, error) {
	...
	for _, c := range conditions {
		built, err := buildFilterCondition(c, onMissing, caseSensitive)
		if err != nil {
			return nil, err
		}
...
func buildFilterCondition(c spec.FilterCondition, onMissing string, caseSensitive bool) (*filter.Condition, error) {
	if len(c.All) > 0 {
		subs, err := buildFilterConditions(c.All, onMissing, caseSensitive)
		if err != nil {
			return nil, err
		}
	...
	if len(c.Any) > 0 {
		subs, err := buildFilterConditions(c.Any, onMissing, caseSensitive)
		if err != nil {
			return nil, err
		}
